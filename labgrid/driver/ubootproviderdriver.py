...
class UBootProviderDriver(Driver):
    ...
    Attributes:
        ...
        scp (str): pathname of System Control Processor (SCP) firmware (Crust)
            used on some sunxi (Allwinner) boards
        opensbi (str): pathname of Open Source Supervisor Binary Interface
        rockchip_tpl (str): pathname of Rockchip TPL blob
        ...
        build_target (str): U-Boot build target to use, instead of the default
            target. This can be useful for boards which need to build extra
            files not covered by the default target
        copy_in (list or str): list of filenames to copy into the source
            directory before building
        device_tree (str): Filename to use for the devicetree. This sets the
    ...
    scp = attr.ib(default='', validator=attr.validators.instance_of(str))
    opensbi = attr.ib(default='', validator=attr.validators.instance_of(str))
    rockchip_tpl = attr.ib(default='', validator=attr.validators.instance_of(str))
    ...
    build_target = attr.ib(default='', validator=attr.validators.instance_of(str))
    copy_in = attr.ib(
        default=attr.Factory(list),
        validator=attr.validators.optional(attr.validators.instance_of(list)),
    )
    device_tree = attr.ib(default='', validator=attr.validators.instance_of(str))
        ...
        try:
            ...
            if b'--fallback-mrproper' in exc.stdout:
                out = processwrapper.check_output(cmd + ['-m'], cwd=cwd,
                                                  env=env)
            elif b'Fail open first container file' in exc.stdout:
    ...
    def build(self, do_print=True, config_only=False):
        ...
        if self.tee:
            env['TEE'] = self.tee
        if self.rockchip_tpl:
            env['ROCKCHIP_TPL'] = self.rockchip_tpl
        if self.device_tree:
            env['DEVICE_TREE'] = self.device_tree
        ...
        if get_var('do-clean', '0') == '1':
            cmd.append('-m')
        if not config_only and self.build_target:
            cmd += ['--target', self.build_target]

        for fname in self.copy_in:
            leaf = os.path.split(fname)[1]
            dst = os.path.join(cwd, leaf)
            if not os.path.exists(leaf):
                self.logger.info(f"copy file '{fname}' to '{cwd}")
